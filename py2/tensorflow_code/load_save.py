...
# These things were intended for an initila approach to save and load TF models.
# However, latere I choose another approach so the stuff here is currently not used.

# The below is taken from TF's "simple_save.py" which exists in later versions of TF.
#
#
...
def save_tf_model(sess, filename):
      tf.train.Saver().save(sess, filename)

def load_tf_model(sess, mdl_dir):
    with tf.Session():
        tf.saved_model.loader.load(sess, [tag_constants.SERVING], mdl_dir)
