...
class tf_pool( object ):
  ...
  def __call__(self, X_, C_, S_=None, res_con=[]):

    ###
    if len(res_con) != 0:
      log.info("Residual connections provided to pool " + str( res_con ) )
      outputs_to_keep = np.unique( [i for l in res_con for i in l ] )
      kept_outputs = {}
    ###
    ...
    def body(M_, i_):
      ...
      for j in range(0, len(self.nn) ):
        if ( len(res_con) > 0):
          #assert (isinstance( self.nn[j], tf_tdnn ) )
          log.debug("Layer " + str(j) + " " + str(self.nn[j]) )
          log.debug("Residuals  " + str(res_con[j]) )
          for o in res_con[j]:
              log.debug("   Adding " +str(o) + " " + str( kept_outputs[ o ] ) )
              #sh1_ = tf.shape(kept_outputs[ o ])
              #sh2_ = tf.shape(Z_ )
              #d_   = [ tf.cast((sh1_[i] - sh2_[i])/2,"int32") for i in range(2) ]
        ...
        if ( ( len(res_con) >0 ) and (j in outputs_to_keep) ):
          log.debug( "Layer " + str(j) + " " + str(self.nn[j]) + " will be stored. " )
          kept_outputs[j] = Z_
